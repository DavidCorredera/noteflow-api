import { NextResponse } from 'next/server';
import { query } from '@/lib/db';
import { verifyAuth } from '@/lib/auth';
import { z } from 'zod';

const itemSchema = z.object({
  text: z.string().min(1, "El texto no puede estar vacío"),
  priority: z.enum(["none", "low", "medium", "high"]).optional()
});

async function verifyNoteOwnership(noteId: string, uid: string): Promise<boolean> {
  const [note] = await query('SELECT id FROM notes WHERE id = $1 AND user_id = $2', [noteId, uid]);
  return !!note;
}

export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const uid = await verifyAuth(request);
    const { id } = await params;

    if (!(await verifyNoteOwnership(id, uid))) {
      return NextResponse.json({ error: 'Nota no encontrada' }, { status: 404 });
    }

    const items = await query('SELECT * FROM checklist_items WHERE note_id = $1 ORDER BY id ASC', [id]);
    return NextResponse.json(items);
  } catch (error: any) {
    if (error.message?.includes('Authorization')) {
      return NextResponse.json({ error: 'No autorizado' }, { status: 401 });
    }
    return NextResponse.json({ error: 'Error interno' }, { status: 500 });
  }
}

export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const uid = await verifyAuth(request);
    const { id } = await params;

    if (!(await verifyNoteOwnership(id, uid))) {
      return NextResponse.json({ error: 'Nota no encontrada' }, { status: 404 });
    }

    const body = await request.json();
    const result = itemSchema.safeParse(body);

    if (!result.success) {
      return NextResponse.json({ errors: result.error.issues }, { status: 400 });
    }

    const [newItem] = await query(
      'INSERT INTO checklist_items (note_id, text, priority) VALUES ($1, $2, $3) RETURNING *',
      [id, result.data.text, result.data.priority ?? 'none']
    );

    return NextResponse.json(newItem, { status: 201 });
  } catch (error: any) {
    if (error.message?.includes('Authorization')) {
      return NextResponse.json({ error: 'No autorizado' }, { status: 401 });
    }
    return NextResponse.json({ error: 'Error interno' }, { status: 500 });
  }
}
