...
export async function listNotes() {
  return query(`${NOTE_WITH_RELATIONS_SELECT} ORDER BY n.created_at DESC;`);
}

export async function getNoteById(id: string) {
  const [note] = await query(`${NOTE_WITH_RELATIONS_SELECT} WHERE n.id = $1;`, [id]);

  return note ?? null;
}

export async function createNoteWithTags(input: NoteWriteInput) {
  const noteId = crypto.randomUUID();
  const tags = normalizeTags(input.tags);

  await sql.transaction([
    sql.query(
      'INSERT INTO notes (id, title, type, content, color) VALUES ($1, $2, $3, $4, $5)',
      [noteId, input.title, input.type, input.content ?? null, input.color ?? null]
    ),
    ...tags.map((tag) =>
